#!/usr/bin/env python3
"""hotflag_h002.py — H-hotflag-002: multi-trigger coincidence as a
confirmation filter, long, 1d (family hotflag, TESTING NIGHT 2026-07-23).

PRE-REGISTRATION lives in
tools/research/families/hotflag/HYPOTHESES.md §H-hotflag-002 — thesis,
...
the script is self-contained: trigger_class == "multi" (2+ coincident
MW_TRIG bits on the same bar), direction long only, horizon fixed at 1d,
frictions 1x/2x/4x. A single config — no grid, no rank-stability
requirement. Same fold recipe and anchor as H-hotflag-001 (shared
calendar) so the two entries' pos_frac are fold-comparable, per the test
plan.

Reuse discipline (rig-is-shared-instrument, NOTES.md 2026-07-20): this
script is H-hotflag-001's script with only TRIGGER_CLASS and output
paths changed — the detector replay, lag-1 fill model, trigger_class
labeling, dedup and cost helpers are imported from hotmkt.py verbatim,
never reimplemented; the EW eligible-universe benchmark and gate wiring
are the same logic H-hotflag-001 introduced, reused verbatim rather than
rewritten a second time.

Usage:
    tools/research/.venv/bin/python \
        tools/research/families/hotflag/hotflag_h002.py
"""
...
RESULTS_PATH = os.path.join(
    rig.DATA_DIR, "hotflag_H-hotflag-002_results.json")
