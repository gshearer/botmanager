...
| Work on plugin lifecycle / hot-reload (`PLIFE-*`), or make a plugin's `deinit()` mirror its `init()` | root `TODO.md` §PLIFE + `core/plugin.c` (`plugin_audit`, `plugin_reclaim`) + `core/cmd.c` (`cmd_unregister_path`). Run `/plugin audit <name>` for the worklist; the verdict is the `plugin_audit` CLAM line at unload. **Class A (commands, KV, clam subscribers, bot KV contributors) is core's job since PLIFE-3** — it reclaims them by registration owner, so a `deinit()` that forgets them is untidy, not unsafe; a plugin's teardown owes only Class B (threads, tasks, in-flight requests, driver vtables) |
...
| Work on the melee duelling game (`!melee`, `show melee`; persona assets `prompts/melee-drow.txt`, `prompts/melee-rpg.txt`) | `plugins/feature/melee/AGENTS.md` |
