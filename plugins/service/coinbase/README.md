# coinbase — Coinbase Exchange Service Plugin

Pure API-connectivity plugin for the Coinbase Exchange (the
former Coinbase Pro / GDAX product, not Advanced Trade or Prime).
Exposes a mechanism API over both REST and a WebSocket feed so
consumers — `plugins/bot/whenmoon/`, future `plugins/cmd/coinbase/`
command surfaces, other service-internal callers — can access
products, candles, live trades, order books, accounts, and place
orders without knowing how Coinbase authentication or streaming
works.
...
- A `PLUGIN_SERVICE` descriptor that loads clean.
- Plugin-level KV schema for endpoint selection, credentials, and
  subsystem toggles.
- Empty public-API header `coinbase_api.h` reserving the type
  shapes and dlsym-shim layout for the full API.
- No REST code, no signing, no WebSocket transport, no command
  surface (commands live in a future `plugins/cmd/coinbase/` or
  inside `plugins/bot/whenmoon/` per the layer rules).

Building the tree produces `libcoinbase.so` and the plugin loads;
nothing else is observable yet. All real functionality lands in
chunks CB1–CB6 in `TODO.md`.
...
1. **Zero user commands.** Coinbase-related `/` commands (if any
   are added) belong either in `plugins/bot/whenmoon/` (when they
