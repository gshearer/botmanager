...
//
// EX-1: candle + trade traffic moved off the direct-curl path onto the
// feature_exchange abstraction. coinbase_fetch_candles_async /
// coinbase_fetch_trades_async now build the path, allocate a typed
// completion context, and dispatch through exchange_request. The
// abstraction's response callback (cb_candles_exchange_resp /
// cb_trades_exchange_resp below) parses the body and fires the user's
// typed cb. Products / ticker / orders / accounts stay on the direct
// path — moving them through the abstraction is deferred until the
// live-trading framework is unpaused.
#define CB_INTERNAL
#include "coinbase.h"
...
// submit handlers in coinbase_exchange.c also need it. The legacy typed
// callers (products / candles / trades / ticker) pass their cb_request_t
// directly; the exchange-vtable path passes its own request handle.
//
...
// the request is released and no callback fires (typed callers invoke
// the matching cb_deliver_*_fail on the FAIL return path below; the
...
// Exchange-routed completion: trades
//
...
static void
cb_trades_exchange_resp(int http_status, const char *body, size_t body_len,
    const char *err, void *user)
{
  cb_request_t             *r   = user;
  coinbase_trades_result_t  res = { 0 };
