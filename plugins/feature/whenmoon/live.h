...
//   (under mk->lock) -> coinbase_place_order_async. Fills land
//   asynchronously via the user WS channel + REST /fills poll.
//
// Master kill-switch lives at `plugin.whenmoon.exchange.coinbase.live`
// (KV_BOOL, default false). Per-market risk caps (daily_loss_bps,
// max_notional, pending_cap) layer on top once the master is enabled.
//
...
// `mk->session.pending[]`, and dispatches coinbase_place_order_async
// at the transactional priority. Caller MUST hold `mk->lock`. Returns
