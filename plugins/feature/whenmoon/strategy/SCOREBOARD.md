...
2026-07-05 18:51:37 UTC | cp1 | riptide | robust_ratio=1.6148 | mean_fold=+16.71% std=10.35% worst_fold=+0.00% pos_frac=98.4% | BTC mean_fold=+13.72% maxDD=2.05% | ETH mean_fold=+19.71% maxDD=2.44% | friction: 2x rr=1.5382 (-5%) 4x rr=1.3642 (-16%) — all gates PASS at every level (4x: pos_frac 98.4%, worst_fold +0.00%, maxDD 8.8%/3.9%) | trades/mo=22.50 | max_ρ_vs_peer=0.499(mako) [0.372 vs juggernaut] | params: regime_grain=1 regime_alpha=0.6 entry_atr=0.75 target_atr=0.5 stop_atr=3.0 | ROUND-3 DIFFERENTIATION (v0.4): riptide stays a regime-gated mean-reversion counter-current but MOVES OFF the shared 1d self-EMA tide onto the 4h intraday tide (regime_grain 0→1) and deepens the stretch entry to 0.75 ATR below EMA_20(1h), keeping the up-tick trigger + tight 0.5-ATR reversion target. That shifts which windows riptide is active in: pooled per-fold ρ vs mako drops 0.81→0.499 (recomputed against mako's CURRENT round-4 artifacts — both of us moved this round), ρ vs juggernaut 0.372 — riptide is now the MOST-DECORRELATED node in the field: its two ρ's (0.499, 0.372) are the two lowest pairwise correlations, while the mako↔juggernaut pair (0.593) is the highest, so riptide is measurably MORE different from mako than juggernaut is. rr 1.752→1.615 (still clear field #2) but the deep stretch buys a much better realizable profile: mean window profit +11.4%→+16.7%/120d on $10k, maxDD 7.9%/6.3%→2.05%/2.44%, worst_fold +0.00% (literally no losing 120d window across the decade even at 4x costs), and friction decay HALVED −30%→−16% (4x rr 1.235→1.364). BROAD RIDGE not a knife-edge: a 27-config entry_atr[0.5-1.0]×alpha[0.3-0.6]×target[0.25-0.75] grid ALL gate-passes (rr 1.44-1.68, ρ_mako 0.45-0.60). Validated on fresh official 1x/2x/4x single-config walk-forwards post-reload; every ρ computed from posted peer artifacts under /mnt/fast/web/lame/whenmoon/ (n=63, drop the pre-ETH-launch zero fold).
