...
bool
wm_dl_candles_dispatch_one(dl_scheduler_t *s, dl_job_t *j)
{
...
wm_dl_candles_on_page(const coinbase_candles_result_t *res, void *user)
{
  ...
  if(bail)
  {
    ...
    // scheduler-wide counter even if the job vanished from the list.
    if(j != NULL)
    ...
    // may be eligible. dispatch_next bails on destroying.
