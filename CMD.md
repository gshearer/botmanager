...
- Command-surface plugin: `plugins/cmd/math/math.c`
...
bool cmd_register(
    ...
    const char *help_long,    // multi-line help (NULL for none)
    ...
    const char *const *kind_filter  // NULL-terminated bot-kind allowlist,
                                    // NULL for kind-agnostic. Misc-plugin
                                    // commands always pass NULL.
);
```
...
mymod_register_commands(void)
{
  cmd_register("mymod", "mycmd",
      ...
      mymod_cmd_handler, NULL, NULL, NULL,
      mymod_cmd_ad, 1,
      NULL);                                  // kind_filter (kind-agnostic)
}
...
## Calling into other plugins

If your command needs to call into another plugin's subsystem (for
example, an LLM completion from the `inference` plugin), declare
that plugin in your descriptor's `.requires` and include its public
shim header — e.g. `#include "inference/inference.h"` — then call
the exported symbols directly. Each shim inline resolves the real
symbol via `plugin_dlsym` on first call and caches it atomically.
`plugins/inference/inference.h` is the reference implementation of
the pattern; see PLUGIN.md §Plugin-to-Plugin Calls for the full
shim shape.
